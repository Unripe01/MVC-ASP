...
画面の表示値は、保存済みXMLを deserialize した結果を正とする。
マスター取得とマスター逆反映は別操作にし、チェックした項目だけを対象にする。
...
帳票Projection対象Entityを serialize / deserialize する。

初期表示では保存済みXMLを deserialize して画面へ出す。

保存時にマスターへ自動逆反映してはいけない。

フォーム入力中の値は編集中の帳票データであり、保存済みXMLとは別に扱う。

一度 `XML保存` した後は、保存済みXMLの内容を画面の正本として扱う。

マスター値を画面へ持ってくるのは「マスター取得」ボタン時のみ、
マスターへ反映するのは「マスター逆反映」ボタン時のみとする。

`マスター取得` と `マスター逆反映` は、どちらもXML保存を伴わない。

マスターの方が新しいと判断した場合は、`マスター取得` の後に内容を確認して `XML保存` する。

XML側の値をマスターへ登録したい場合だけ `マスター逆反映` を使う。
